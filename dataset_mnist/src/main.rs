...
impl NeuralNetwork {
    ...
    pub fn train(&mut self, train_data: &[(Vec<f64>, usize)], val_data: &[(Vec<f64>, usize)], epochs: usize, batch_size: usize) -> (Vec<f64>, Vec<f64>, Vec<f64>) {
        ...
        for epoch in 0..epochs {
            ...
            for batch in shuffled_data.chunks(batch_size) {
                ...
                for (sample_input, target) in batch {
                    let prediction = self.forward(sample_input);
            ...
            // Validación
            let (val_loss, val_accuracy) = self.validate(val_data);
    ...
    /// Valida el modelo
    pub fn validate(&mut self, val_data: &[(Vec<f64>, usize)]) -> (f64, f64) {
...
/// Función principal optimizada para datasets pequeños
