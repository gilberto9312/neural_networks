...
fn main() {
    ...
    // 1. Sin regularización
    {
        ...
        results.push(("Sin regularización", train_err, val_err, converged, epochs));
    }
    ...
    // 2. Solo L1
    {
        ...
        results.push(("L1 (λ=0.01)", train_err, val_err, converged, epochs));
    }
    ...
    // 3. Solo L2
    {
        ...
        results.push(("L2 (λ=0.01)", train_err, val_err, converged, epochs));
    }
    ...
    // 4. Solo Dropout
    {
        ...
        results.push(("Dropout (50%)", train_err, val_err, converged, epochs));
    }
    ...
    // 5. L2 + Dropout (combinado)
    {
        ...
        results.push(("L2 + Dropout", train_err, val_err, converged, epochs));
    }
    
    // Resumen final
    println!("\n📈 RESUMEN FINAL");
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    for (name, train_err, val_err, converged, epochs) in &results {
        let status = if *converged { "✅" } else { "❌" };
        println!("🔹 {} {} → Épocas: {}, Val Error: {:.6}", 
                 name, status, epochs, val_err);
    }
    
    // Mostrar el mejor
    if let Some(best) = results.iter()
        .filter(|(_, _, _, converged, _)| *converged)  // Solo los que convergieron
        .min_by(|(_, _, val_err1, _, epochs1), (_, _, val_err2, _, epochs2)| {
            val_err1.partial_cmp(val_err2).unwrap_or(epochs1.cmp(epochs2))
        }) {
        println!("\n🏆 MEJOR RESULTADO: {} con error {:.6} en {} épocas", 
